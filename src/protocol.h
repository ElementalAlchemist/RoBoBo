...
class Protocol {
	public:
		...
		virtual std::string userMetadata(std::string user, std::string key);
		...
		virtual std::tr1::unordered_map<std::string, std::tr1::unordered_map<std::string, time_t> > listXLines(); // unordered_map { line type => unordered_map { hostmask => time of expiry } }
