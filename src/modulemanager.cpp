...
void ModuleManager::pointManagers(ServerManager* sm, SocketManager* sockm) {
	servers = sm;
	sockets = sockm;
}
...
std::shared_ptr<Module> ModuleManager::openModule(const std::string& name) {
	...
	Module*(*spawnCallFunc)(const std::string&) = reinterpret_cast<Module*(*)(const std::string&)>(spawnFunc);
...
void ModuleManager::verifyModule(const std::string& name, std::shared_ptr<Module> mod) {
	...
	for (auto hook : actions) {
		...
		if (beforeIter == actionList.end() && afterIter == actionList.rend()) {
